...
## 7. CÔNG NGHỆ PHÁT TRIỂN HỆ THỐNG (TECH STACK)

Trong việc báo cáo và thuyết trình dự án, phần công nghệ phát triển giúp minh chứng năng lực kỹ thuật và giải thích cơ sở khoa học đằng sau các tính năng vận hành. Hệ thống được phát triển dựa trên các công nghệ và công cụ tối ưu sau:

### 1. Phân hệ Backend & Cơ sở dữ liệu (Trọng tâm xử lý nghiệp vụ)

* **Framework chính: Laravel (PHP)**
  * *Lý do:* Cung cấp sẵn hệ thống lập lịch tác vụ (`Artisan Console`, `Scheduler`) mạnh mẽ giúp tự động hóa việc kiểm kho, tạo báo cáo và backup dữ liệu hàng đêm một cách chính xác mà không cần cấu hình thủ công phức tạp.
* **Cơ sở dữ liệu: MySQL**
  * *Lý do:* Đảm bảo tính toàn vẹn dữ liệu cao (ACID) khi xử lý các giao dịch tài chính (thanh toán, tách hóa đơn) và quản lý chặt chẽ cấu trúc các lô hàng nhập.

### 2. Giao tiếp Thời gian thực (Real-time Communication)

* **Công nghệ kết nối: Laravel Reverb & Laravel Echo (WebSockets)**
  * *Lý do:* Đây là "xương sống" giúp hệ thống đồng bộ dữ liệu tức thời giữa Khách đặt món $\rightarrow$ Nhà bếp $\rightarrow$ Thu ngân mà không cần F5 (tải lại trang), triệt tiêu độ trễ vận hành.

### 3. Phân hệ Frontend (Giao diện người dùng)

* **Giao diện & Tương tác: HTML5, CSS3, JavaScript (kết hợp Bootstrap / Tailwind CSS)**
  * *Lý do:* Giúp giao diện hiển thị mượt mà trên mọi thiết bị, từ điện thoại của khách quét mã QR cho đến máy tính bảng của đầu bếp, tối ưu hóa các sơ đồ bàn ăn trực quan và màn hình KDS.

### 4. Các thư viện bổ trợ tích hợp (Packages)

* **Xuất báo cáo tài chính: Laravel Excel (Maatwebsite/Excel)**
  * *Lý do:* Hỗ trợ xuất file Excel báo cáo định kỳ cuối tháng định dạng UTF-8, tự động vẽ biểu đồ sản lượng bán hàng (in-cell bar chart) trực quan trực tiếp vào file.
* **Quản lý Mã QR: Simple QrCode (Laravel Package)**
  * *Lý do:* Tự động sinh mã QR động chứa thông tin bàn ăn để in hàng loạt hoặc hiển thị mã QR ngân hàng khi khách yêu cầu thanh toán chuyển khoản.
