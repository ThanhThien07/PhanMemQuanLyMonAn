...
class NguyenLieuController extends Controller
{
    ...
    public function comparePrice(Request $request): JsonResponse
    {
        ...
        if (stripos($search, 'bò') !== false) {
            $basePrice = 200000;
        } else if (stripos($search, 'bơ') !== false) {
            $basePrice = 140000;
        } else if (stripos($search, 'sữa') !== false) {
            $basePrice = 35000;
        }
        ...
        $dbSuppliers = \App\Models\NhaCungCap::all();
        if ($dbSuppliers->isEmpty()) {
            $dbSuppliers = collect([
                new \App\Models\NhaCungCap(['ten' => 'GlobalFood Import Ltd']),
                new \App\Models\NhaCungCap(['ten' => 'AsiaImport Co.']),
                new \App\Models\NhaCungCap(['ten' => 'EuroIngredient Group']),
            ]);
        }
